    ...
    , ui(new Ui::DialogConfigBooks)
{
    ...
    ui->toolButtonUpdatBk->setIcon(
        QIcon(style()->standardPixmap(QStyle::SP_BrowserReload)));
    ui->toolButtonGroupUpdat->setIcon(
        QIcon(style()->standardPixmap(QStyle::SP_BrowserReload)));
...
DialogConfigBooks::~DialogConfigBooks() { delete ui; }
...
void DialogConfigBooks::on_toolButtonGroupTosection_clicked()
{
    ...
    int index = ui->treeWidgetCurGroup->indexOfTopLevelItem(
        ui->treeWidgetCurGroup->currentItem());
...
void DialogConfigBooks::on_toolButtonBkPath_clicked()
{
    ...
    QString dir = dlg.getExistingDirectory(
        this, tr("Open Directory"), ui->lineEditPath->text(),
...
void DialogConfigBooks::on_toolButtonFindBKOld_clicked()
{
    ...
    QString dir = dlg.getExistingDirectory(
        0, tr("Open Directory"), ui->lineEditPath->text(),
        QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
    if (dir.isEmpty())
        return;
...
void DialogConfigBooks::addbook(QTreeWidgetItem* itemNew, QTreeWidgetItem* item,
    QString name)
{
    bool exist = false;
...
void DialogConfigBooks::on_treeWidgetCurGroup_itemActivated(QTreeWidgetItem,
    int)
{
    on_treeWidgetCurGroup_itemSelectionChanged();
}

void DialogConfigBooks::on_treeWidgetCurGroup_itemClicked(QTreeWidgetItem,
    int)
{
    on_treeWidgetCurGroup_itemSelectionChanged();
}
