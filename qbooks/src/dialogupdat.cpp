    ...
    , ui(new Ui::dialogupdat)
{
    ...
    connect(ui->lineEdit_autor, SIGNAL(textEdited(QString)), this,
        SLOT(okEnabled()));
    connect(ui->lineEdit_booknam, SIGNAL(textEdited(QString)), this,
        SLOT(okEnabled()));
    connect(ui->textBrowser, SIGNAL(undoAvailable(bool)), this,
        SLOT(okEnabled()));
    connect(ui->comboBox_group, SIGNAL(currentIndexChanged(int)), this,
        SLOT(okEnabled()));
}
