...
bool Messages::fahrasSave(QTreeWidget* view, QString bkname)
{
    ...
    QList<QTreeWidgetItem*> found = view->findItems(
    ...
    if (found.count() > 0) {
        ...
        foreach (item, found) {
            ...
            if (!item->parent()) { // اذا كان العنصر جذرا
                ...
                QVariant lvlstr = lvl;
                writeInDoc(item->text(0), item->data(1, 1).toString(),
                    lvlstr.toString());
                lvl = 1;
            }
        }
...
bool Messages::treeviewItemUp(QTreeWidget* view)
{
    ...
    if (!curent_item->parent()) { // اذا كان في قمة الشجرة اي ليس له ابناء
        ...
        view->insertTopLevelItem(
            t - 1, new_item); // اظافة العنصر المنسوخ في الذاكرة بعد العنصر التالي
...
bool Messages::treeviewItemDown(QTreeWidget* view)
{
    ...
    if (!curent_item || !old_item) {
        return false;
    } // اذا لم يوجد عنصرا محددا او التالي يخرج
    new_item = curent_item->clone(); // نسخ العنصر المحدد مع ابنائه ان كان يحمل ابناء
    if (!curent_item->parent()) { // اذا كان في قمة الشجرة اي ليس له ابناء
        t = view->indexOfTopLevelItem(curent_item); // معرفة ترقيم العنصر
...
bool Messages::loadTarGz(QString path)
{
    ...
    prosses.waitForFinished();
    prosses.start(QString("tar -xvf  %1 -C  %2")
                      .arg("\"" + path + "\"")
                      .arg("\"" + pathToExtract + "\""));
    while (prosses.state() == QProcess::Running) {
        qApp->processEvents();
    ...
    prosses.start(
        QString("mv   %1   %2")
            .arg("\"" + pathToExtract + "/" + archiveDir + "/book.xml\"")
            .arg("\"" + pathToExtract + "\""));
    prosses.waitForFinished();
    prosses.start(
        QString("mv   %1   %2")
            .arg("\"" + pathToExtract + "/" + archiveDir + "/bookinfo.info\"")
            .arg("\"" + pathToExtract + "\""));
    prosses.waitForFinished();
    prosses.start(
        QString("mv   %1   %2")
            .arg("\"" + pathToExtract + "/" + archiveDir + "/title.xml\"")
            .arg("\"" + pathToExtract + "\""));
    prosses.waitForFinished();
    return true;
#endif
