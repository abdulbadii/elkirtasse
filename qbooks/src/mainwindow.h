...
private slots:
    ...
    //**treeViewBook**
    void
    on_treeWidget_books_itemSelectionChanged(); // عند تغير التحديد في شجرة الكتب
    void on_treeWidget_books_itemActivated(
        QTreeWidgetItem* item); // تفعيل عنصر شجرة  الكتب
    void
    on_treeWidget_books_customContextMenuRequested(); // انشاء قائمة لشجرة الكتب
    ...
    //**treeVeiwCuran**
    void on_treeWidget_curaan_itemActivated(
        QTreeWidgetItem* item); // تفعيل عنصر شجرة القرءان الكريم
    ...
    void open_dataBase(QString name, QString title, QString Autor,
        int position); // فتح الكتاب المحدد
