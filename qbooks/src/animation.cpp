    ...
    if (level == 1) {
        ...
        for (int i = 0; i < topItemCount; ++i) {
            ...
            for (int r = 0; r < nbr; r++) {
                ...
                p.drawText(8, 8, 112, 52, Qt::AlignCenter | Qt::TextWordWrap,
                    osloItem->text(0));
        ...
        for (int i = 0; i < nbr; i++) {
            ...
            p.setPen(Qt::white);
            p.setFont(QFont("Arial", 14));
            p.drawText(5, 15, 100, 100, Qt::AlignCenter | Qt::TextWordWrap,
                osloItem->text(0));
            p.end();
    ...
    if (level == 1) {
        ...
        for (int i = 0; i < topItemCount; ++i) {
            ...
            for (int r = 0; r < nbr; r++) {
                ...
                p.drawText(15, 16, 228, 80, Qt::AlignCenter | Qt::TextWordWrap,
                    osloItem->text(0));
                ...
                p.end();
                Pixmap* item = new Pixmap(mypixmap);
                //                 QGraphicsColorizeEffect *efet=new
                ...
                //                connect(item, SIGNAL(enter()), this,
                //                SLOT(ThreeDEnterItem())); connect(item,
                //                SIGNAL(leave()), this, SLOT(ThreeDLeaveItem()));
            }
        }
        //--------------------------------------------------------
    } else if (level == 2) {
        ...
        for (int i = 0; i < nbr; i++) {
            ...
            // p.setPen(Qt::white);
            p.setFont(QFont("Arial", 14));
            p.drawText(24, 18, 190, 100, Qt::AlignCenter | Qt::TextWordWrap,
                osloItem->text(0));
            p.setFont(QFont("Arial", 10));
            p.drawText(24, 100, 190, 178,
            ...
            //            connect(item, SIGNAL(enter()), this,
            //            SLOT(ThreeDEnterItem())); connect(item, SIGNAL(leave()),
            //            this, SLOT(ThreeDLeaveItem()));
        }
    }
    chargeMaskItem();
    chargeNavigator();
...
void View::animTeled()
{
    ...
    for (int i = 0; i < items.count(); ++i) {
        ...
        anim->setEndValue(QPointF(((i % 6)) * 128, ((i / 6)) * 128 + 20));
    }
    group->start();
    itemCurant = 0;
#else
    for (int i = 0; i < items.count(); ++i) {
        items[i]->setPos(QPointF(((i % 7)) * 128, ((i / 6)) * 128 + 20));
    }
#endif
}
...
void View::moveThreeD(bool right)
{
    ...
    for (int i = 0; i < 7; ++i) {
        ...
        anim1->setEndValue(
            QPointF(leftCentre + (posLleft + s), topCentre + (d * 30)));
...
{
    Pixmap* currentItem = qobject_cast<Pixmap*>(sender());
    //  currentItem->setGeometry(QRect(currentItem->pos().x()-5,
    //  currentItem->pos().y()-5, 138, 138));
    currentItem->setTransform(
        QTransform().translate(0, 0).scale(1.07, 1.07).translate(-4, -4));
}
void View::animLeaveItem()
{
    Pixmap* currentItem = qobject_cast<Pixmap*>(sender());
    // currentItem->setGeometry(QRect(currentItem->pos().x()+5,
    // currentItem->pos().y()+5, 128, 128));
    currentItem->setTransform(
        QTransform().translate(0, 0).scale(1, 1).translate(0, 0));
}
...
void View::stop_time() { m_timer->stop(); }
...
void View::chargeRecent()
{
    ...
    if (threeD == false) {
        ...
        for (int i = 1; i < recentNbr; i++) {
            ...
            p.begin(&mypixmap);
            p.setFont(QFont("Arial", 18));
            p.drawText(24, 18, 190, 178, Qt::AlignCenter | Qt::TextWordWrap,
                recentFileActs[i]->text());
...
void View::keyPressEvent(QKeyEvent* event)
{
    ...
    if (isThreeD == true) {
        ...
        //        items[itemCurant]->setTransform(QTransform().translate(0,0).scale(1,1).translate(0,
        //        0));
        //         if (event->key() == Qt::Key_Right ||event->key() == Qt::Key_Down
        //         ){
        //            itemCurant=itemCurant+1;
        //            if (itemCurant>items.count()){ itemCurant=items.count()-1; }
        //         }else if(event->key() == Qt::Key_Left ||event->key() ==
        //         Qt::Key_Up){
        //             itemCurant=itemCurant-1;
...
فمن كانت هجرته إلى دنيا يصيبها، أو إلى امرأة ينكحها، فهجرته إلى ما هاجر
إليه")+"</span></p></td></tr></table></body></html>";
    ...
    for (int i = 0; i < items.count(); ++i) {
        r=r+36;
        QPropertyAnimation * anim = new QPropertyAnimation(items[i],
"rotation"); anim->setStartValue(-r); anim->setEndValue(-360);
    ...
    for (int i = 0; i < items.count(); ++i) {
        ...
        QPropertyAnimation * anim = new QPropertyAnimation(items[i],
"transformOriginPoint"); anim->setEndValue(QPointF(64,64));
...
"rotation"); anim->setStartValue(r); anim->setEndValue(360);
    ...
    //! [5]
    QPropertyAnimation * animTitle = new QPropertyAnimation(itemTitle,
"opacity"); animTitle->setDuration(1000); animTitle->setStartValue(0.0);
...
void View::chargeEnrto()
{
    ...
    QString text = "<p align=\"center\" dir='rtl' style=\" margin-top:0px; "
        ...
        + tr("حدثنا الحميدي عبد الله بن الزبير قال: حدثنا سفيان قال: حدثنا "
                 ...
                 "الأعمال بالنيات، وإنما لكل امرىء ما نوى، فمن كانت هجرته إلى دنيا "
...
void View::animEtoileTitle() { animEtoile(QRect(155, 190, 32, 32)); }
void View::animEtoileBook() { animEtoile(QRect(265, 60, 32, 32)); }
