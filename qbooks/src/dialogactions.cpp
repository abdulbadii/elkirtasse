...
DialogActions::~DialogActions() { delete ui; }
...
void DialogActions::on_toolButtonAdd_clicked()
{
    ...
    item = curItem->clone();
    if (curItemDest) {
        ui->treeWidgetDest->insertTopLevelItem(
            ui->treeWidgetDest->indexOfTopLevelItem(curItemDest) + 1, item);
    } else {
...
void DialogActions::setListActions(QList<QAction*>* listActions,
    QList<QVariant> listToAdd)
{
...
void DialogActions::on_toolButtonDown_clicked()
{
    ...
    int index = ui->treeWidgetDest->indexOfTopLevelItem(curItem);
    if (index == ui->treeWidgetDest->topLevelItemCount() - 1)
        return;
    QTreeWidgetItem* item = new QTreeWidgetItem;
    item = curItem->clone();
