...
void Dialognet::treeChargeGroupe(QTreeWidget* view, int checked)
{
    ...
    QIcon icon2;
    icon2.addPixmap(QPixmap(QString::fromUtf8(":/images/image/groupb.png")),
        QIcon::Normal, QIcon::Off);
...
{
    if (downloadFile(ui->lineEdit->text(),
            QDir::homePath() + "/.kirtasse/data/")
        == true) {
...
bool Dialognet::downloadFile(QString urlPath, QString distPath)
{
    ...
    connect(reply, &QNetworkReply::downloadProgress, this,
        &Dialognet::updateDataReadProgress);
...
void Dialognet::httpRequestFinished(QNetworkReply* reply)
{
    ...
    if (reply->error() != QNetworkReply::NoError) {
        ...
        QMessageBox::information(
            0, tr("HTTP"), tr("Download failed: %1.").arg(reply->errorString()));
        httpRequestAborted = true;
    } else {
        QString fileName = QFileInfo(QUrl(m_urlPath).path()).fileName();
        //    QMessageBox::information(0, tr("HTTP"),tr("Downloaded %1  to
        //    %2").arg(fileName).arg(QDir::homePath())+"/.kirtasse/data/");
        mypath = fileName;
        isloaded = true;
        httpRequestAborted = true;
    }
...
#ifndef QT_NO_OPENSSL
void Dialognet::sslErrors(QNetworkReply* reply,
...
////--------------------------
void Dialognet::searchInTreeview(QTreeWidget* view, QString searchString,
    int colum)
{
    for (int i = 0; i < view->topLevelItemCount(); i++) {
    ...
    if (!searchString.isEmpty()) {
        for (int i = 0; i < view->topLevelItemCount(); i++) {
            searchTreeForString(searchString, view->topLevelItem(i), true, i, view,
                colum);
        }
    }
}

bool Dialognet::searchTreeForString(const QString& searchString,
    QTreeWidgetItem* parent, bool itemtop,
    int topindex, QTreeWidget* view,
    int colum)
{
    int hiddenCount = 0;
    int hiddenparent = 0;
    ...
    for (int i = 0; i < parent->childCount(); ++i) {
        ...
        if (parent->isHidden())
            hiddenCount++;
        else if (child->childCount() > 0 && !child->text(colum).contains(searchString, Qt::CaseInsensitive)) { // لا يحمل النص وله ابناء
            if (!searchTreeForString(searchString, child, false, topindex, view,
                    colum)) {
                child->setHidden(true);
                hiddenCount++;
            }
        } else if (child->childCount() > 0 // يحمل النص  له ابناء
            && (child->text(colum).contains(searchString,
                Qt::CaseInsensitive))) {
