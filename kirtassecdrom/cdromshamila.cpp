...
QString cdromShamila::execPlugin()
{
    ...
    if (dlg->exec() == QDialog::Accepted) {
        ...
        if (creatShamellaWin() == true) {
            ...
            QFile::rename(QDir::homePath() + "/.kirtasse/data/group.xml",
                QDir::homePath() + "/.kirtasse/data/group.xml.old");
            QFile::copy(m_tempDir + "/0cat.xml",
                QDir::homePath() + "/.kirtasse/data/group.xml");
...
bool cdromShamila::winCreatgroupXml()
{
    ...
    m_db.setDatabaseName(
        "DRIVER={Microsoft Access Driver (*.mdb)};FIL={MS Access};DBQ=" + m_pathMain);
...
bool cdromShamila::addNewBook(QString bkpath, QString bktitle, QString bkauth,
    ...
    while (!noeud.isNull()) // كلما كان العنصر غير فارغا
    {
        ...
        if (groupPer.tagName() == "root") // اذا كان اسم العنصر "root"
        {
            ...
            for (int i = 0; i < (int)tab.length(); ++i) // عدد الابناء
            {
                ...
                if (mygroup.attribute("id") == groupid) // التوقف عند العنصر الذي اخترناه من المجموعة مثلا
                                                        // "التفاسير
                {
                    QDomElement myel = m_doc.createElement("bk"); // انشاء عنصر جديد
                    mygroup.appendChild(myel); // اظافته في اخر القائمة
                    myel.setAttribute("id",
                        "bk" + groupid + "_" + bkpath); // اسم مجلد الكتاب
...
bool cdromShamila::creatBook(QString id, QString cat)
{
    ...
    if (file.exists(fileMdb)) {
        ...
        qApp->processEvents();
        if (creatXmlFile(newBookPath + "/book.xml", "book",
                m_tempDir + "/tempbk.csv")
            == false)
            return false;

        creatXmlFile(newBookPath + "/title.xml", "title",
            m_tempDir + "/temptitle.csv");
...
bool cdromShamila::saveBookInfo(QString bookname, QString title, QString author,
    QString betaka)
{
    QFile file;
